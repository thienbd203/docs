...
Middleware `Laravel\Passport\Http\Middleware\CheckTokenForAnyScope` có thể được gán cho một route để xác minh rằng access token của yêu cầu đến có _ít nhất một_ trong các scopes được liệt kê:
